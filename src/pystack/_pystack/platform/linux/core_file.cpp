...
#include "platform/linux/core_file.h"
#include <csignal>
#include <gelf.h>
...
void
LinuxCoreFileExtractor::populateMaps()
{
...
LinuxCoreFileExtractor::LinuxCoreFileExtractor(std::shared_ptr<AbstractCoreFileAnalyzer> analyzer)
...
LinuxCoreFileExtractor::MemoryMaps() const
{
    return d_maps;
}

std::vector<SimpleVirtualMap>
LinuxCoreFileExtractor::ModuleInformation() const
{
    return d_module_info;
}

pid_t
LinuxCoreFileExtractor::Pid() const
{
    return d_dwfl_analyzer->getPid();
}

std::string
LinuxCoreFileExtractor::extractExecutable() const
{
...
std::vector<CoreVirtualMap>
LinuxCoreFileExtractor::extractMappedFiles() const
{
...
CoreCrashInfo
LinuxCoreFileExtractor::extractFailureInfo() const
{
    Elf* elf = d_dwfl_analyzer->getElf().get();
...
CorePsInfo
LinuxCoreFileExtractor::extractPSInfo() const
{
    Elf* elf = d_dwfl_analyzer->getElf().get();
    CorePsInfo result{};
...
uintptr_t
LinuxCoreFileExtractor::findExecFn() const
{
...
LinuxCoreFileExtractor::missingModules() const
{
...
{
    return std::make_unique<LinuxCoreFileExtractor>(std::move(analyzer));
}
