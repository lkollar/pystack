...
class UnixRemoteMemoryManager : public AbstractRemoteMemoryManager
{
  public:
    explicit UnixRemoteMemoryManager();
    explicit UnixRemoteMemoryManager(const std::vector<VirtualMap>& vmaps);
  ...
  protected:
    virtual ssize_t readChunk(remote_addr_t addr, size_t len, char* dst) const = 0;
